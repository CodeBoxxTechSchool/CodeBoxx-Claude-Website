...
const SEED_POSTS = [
  {
    ...
    date: '2026-01-14',
    excerpt: 'Corporate AI training has become one of the fastest-growing investments in workforce development. Most organizations still struggle to find programs that go beyond awareness and actually change how teams work.',
  ...
  },
  {
    ...
    author: 'Codeboxx Technology',
    date: '2025-12-01',
    excerpt: 'Four days, virtual or in the St. Petersburg classroom, ending with a product of yours in production or in your portfolio.',
  ...
  },
  {
    ...
    author: 'Codeboxx Technology',
    date: '2025-11-18',
    excerpt: 'A referral partner program that lets individuals, businesses and organizations across Tampa Bay connect motivated learners to the Academy.',
  ...
  },
  {
    ...
    date: '2025-10-22',
    excerpt: 'The University of South Florida Office of Corporate Training and Professional Education launches a new course offering for professionals investing in their growth.',
...
function Posts() {
  ...
  React.useEffect(() => { setShown(PAGE_SIZE); }, [cat]);
  React.useEffect(() => {
    if (!more || !sentinel.current) return;
    const io = new IntersectionObserver(e => { if (e[0].isIntersecting) setShown(s => s + PAGE_SIZE); }, { rootMargin: '200px' });
    ...
    <section className="sect">
      <div className="wrap" style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {CATEGORIES.map(c => (
            <button key={c} onClick={() => setCat(c)} style={{ appearance: 'none', cursor: 'pointer', border: 0, borderRadius: 9999, padding: '8px 16px', fontFamily: 'inherit', fontSize: 12, fontWeight: 600, letterSpacing: '0.5px', textTransform: 'uppercase', background: cat === c ? 'var(--blue-500)' : 'transparent', color: cat === c ? 'var(--neutral-0)' : 'var(--ui-slate-500)', boxShadow: cat === c ? 'none' : 'inset 0 0 0 1px var(--ui-slate-200)' }}>{c}</button>
          ))}
        </div>
        <div className="grid2">
          {list.map(p => (
            <article key={p.title} className="panel" style={{ gap: 16, padding: 0, overflow: 'hidden' }}>
              <image-slot id={'post-' + p.title.toLowerCase().replace(/[^a-z0-9]+/g, '-').slice(0, 40)} shape="rect" fit="cover" placeholder="Cover image" style={{ width: '100%', height: 200 }}></image-slot>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12, padding: '0 24px 24px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16 }}>
                  ...
                  <span style={{ fontSize: 11, fontWeight: 500, color: 'var(--ui-slate-400)' }}>{fmt(p.date)}</span>
                </div>
                <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700, lineHeight: '124%', textWrap: 'pretty' }}>{p.title}</h2>
                <p className="pbody">{p.excerpt}</p>
                <div className="rule" />
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16 }}>
                  <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--ui-slate-500)' }}>{p.author}</span>
                  <a href={p.url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 11, fontWeight: 600, letterSpacing: 1, textTransform: 'uppercase' }}>Read Post</a>
                </div>
              </div>
            </article>
          ))}
        ...
        {more ? (
          <div ref={sentinel} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
            <Button variant="secondary" size="lg" onClick={() => setShown(s => s + PAGE_SIZE)}>Load More</Button>
            <span style={{ fontSize: 11, fontWeight: 500, color: 'var(--ui-slate-400)' }}>{'Showing ' + list.length + ' of ' + all.length}</span>
          </div>
        ) : null}
      </div>
    </section>
    ...
    <section className="sect" style={{ background: 'var(--neutral-0)', boxShadow: 'inset 0 1px 0 0 var(--ui-slate-200)' }}>
      <div className="wrap grid2" style={{ alignItems: 'start' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <p className="lede">Curriculum updates, workshop dates and technology notes. One email per post, no digests.</p>
        </div>
        <div className="panel">
          <Input id="blog-email" label="Email" placeholder="name@company.com" style={{ width: '100%' }} />
          <div className="rule" />
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16 }}>
            <span style={{ fontSize: 12, fontWeight: 500, color: 'var(--ui-slate-400)' }}>Unsubscribe from any email.</span>
...
function BlogPage() {
  return (
    <div id="top">
      <TopBar onCodi={() => { window.location.href = '/#contact'; }} />
      <Band />
      <Posts />
      <Subscribe />
      <Footer />

...
built with Vite and the CodeBoxx design system. Blog content is fetched at runtime from EmDash.
...
- **EmDash** CMS, fetched at runtime (`src/lib/emdash.js`)
...
npm install
cp .env.example .env   # fill in the EmDash values
npm run dev
```
...
| `/blog` | `src/pages/Blog.jsx` | CodeBlog index, EmDash-backed |
| `/financing` | `src/pages/Financing.jsx` | Academy financing options |
| `/ventures` | `src/pages/Ventures.jsx` | CodeBoxx Ventures |

## EmDash

`src/lib/emdash.js` exposes `fetchCollection(name, params)` and the `useEmdashPosts(seed)`
hook the Blog page uses. Without `VITE_EMDASH_API_URL` the page renders the seed array, so
the site still builds and runs with no CMS connection.

Entry mapping lives in `toPost()` — adjust the field names there if the EmDash `post`
collection uses a different schema.
...
npm run dev
git add -A
git commit -m "feat: replace boilerplate with CodeBoxx'27 site (Vite + React + SCSS + EmDash)"
